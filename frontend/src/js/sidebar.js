...
export function setupSidebarNavigation() {
  ...
  // Añadir elemento "Cerrar sesión" al final del sidebar y configurar su comportamiento
  const sidebarMenu = document.querySelector(".sidebar-menu");
  if (sidebarMenu) {
    // Evitar duplicados si ya existe
    if (!document.getElementById("sidebar-logout")) {
      const logoutLi = document.createElement("li");
      logoutLi.id = "sidebar-logout";
      logoutLi.className = "d-flex align-items-center";
      logoutLi.style.cursor = "pointer";
      // Empujar hacia abajo si el menú está en columna (esto funciona cuando el contenedor padre es flex column)
      logoutLi.style.marginTop = "auto";
      logoutLi.innerHTML =
        '<i class="bi bi-box-arrow-right me-2"></i>Cerrar sesión';

      logoutLi.addEventListener("click", function () {
        // Eliminar token de localStorage y redirigir a la página de autenticación
        localStorage.removeItem("token");
        // Redirigir a auth.html (ubicación consistente con otras rutas)

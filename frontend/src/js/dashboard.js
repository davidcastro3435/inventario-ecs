...
async function mostrarValorTotalInventario() {
  try {
    ...
    if (valorBox) {
      valorBox.textContent = total.toLocaleString("es-ES", {
        style: "currency",
        currency: "CRC",
      });
    }
...
async function renderStockMensualLineChart() {
  try {
    ...
    const ctx = document
      .getElementById("stockMensualLineChart")
      .getContext("2d");
    new Chart(ctx, {
      type: "line",
      data: {
...
async function renderInventoryCategoryChart() {
  ...
  const ctx = document
    .getElementById("inventoryCategoryChart")
    .getContext("2d");
  new Chart(ctx, {
    type: "doughnut",
    data: {
...
function renderLowStockTable() {
  ...
  for (const item of lowStockItems) {
    ...
    const nombre = item.nombre ?? item.descripcion ?? `ID ${id}`;
    const stock = item.stock_actual ?? "";
    const minimo = item.alarma ?? "";
